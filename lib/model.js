...
(function () {
    ...
    this.setupField = function (field) {
        ...
        if (this.definition[field].hasOwnProperty('model')) {
            ...
            if (submodel instanceof VeryModel) {
                ...
                this.definition[field].collection = submodel.options.name;
            } else if (typeof submodel === 'string') {
                this.definition[field].deferredCollection = submodel;
                this.definition[field].collection = submodel;
            } else {
                this.definition[field].subVeryCollection = new VeryModel(submodel);
                this.definition[field].collection = '';
            }
        }
    ...
    this.exportJoi = function (fields, unknown) {
        ...
        fields.forEach(function (field) {
            ...
            if (this.definition[field].hasOwnProperty('validate') && this.definition[field].validate.isJoi) {
                ...
                if (this === this.definition[field].subModel) {
                    obj[field] = joi.object().unknown();
                } else {
                ...
                if (this === this.definition[field].subVeryCollection) {
                    obj[field] = joi.array().items(joi.object().unknown());
                } else {
     ...
     */
    this.create = function (value, opts) {
        var model = {};
        var meta = {};
        var obj;
        var factory = this;
        ...
        Object.keys(this.map).forEach(function _createField(field) {
            ...
            if (def.hasOwnProperty('deferredCollection')) {
                ...
                def.collection = submodel.options.name;
            ...
            model.__defineSetter__(field, function (value) {
                ...
                if (!meta.creating && typeof meta.data[mapfield] !== 'undefined' && value !== meta.data[mapfield] && !meta.old_data.hasOwnProperty(mapfield)) {
        ...
        //works recursively
        model.loadData = function (value, process, processors) {
            ...
            if (Array.isArray(meta.defs)) {
                ...
                Object.keys(value).forEach(function (key) {
                    ...
                    processors.forEach(function (pflag) {
                        if (def.hasOwnProperty('processors') && typeof def.processors[pflag] === 'function') {
                            value[key] = def.processors[pflag](value[key], this);
                        }
                    }.bind(this));
                    ...
                    if (def.hasOwnProperty('collection')) {
                        ...
                        meta.data[mapkey].loadData(value[key], process, processors);
                    } else {
                        meta.isEmpty = false;
                        if (processIn) {
        ...
        model.toJSON = function (opts) {
            ...
            Object.keys(meta.defs).forEach(function (field) {
                ...
                var key = field;
                var def = meta.defs[field];
                if (def.private && !opts.withPrivate) {
                    return;
                }
                ...
                opts.processors.forEach(function (pflag) {
                    if (def.hasOwnProperty('processors') && typeof def.processors[pflag] === 'function') {
                        obj[key] = def.processors[pflag](obj[key], this);
                    }
                }.bind(this));
                
                if (!opts.noDepth && def.hasOwnProperty('model')) {
                    if (!meta.data[field].__verymeta.isEmpty) {
                        obj[key] = meta.data[field].toJSON(opts);
                    } else if (def.showEmpty === true) {
                        obj[key] = {};
                    }
                    return;
                ...
                if (!opts.noDepth && def.hasOwnProperty('collection')) {
                    obj[key] = [];
                    meta.data[field].forEach(function (inst) {
                        if (!inst.__verymeta.isEmpty) {
                            obj[key].push(inst.toJSON(opts));
                        }
                    });
                    if (obj[key].length === 0 && !def.showEmpty) {
                        delete obj[key];
                    }
                ...
                if (processOut) obj[key] = processOut(this[field]);
        ...
        if (typeof value !== 'undefined') {
            model.loadData(value, undefined, opts.processors);
        }
